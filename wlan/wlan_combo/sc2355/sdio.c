...
void sprd_chr_get_cmdval(u32 *val, u8 *pos, u8 *key, int octal)
{
...
int sprd_chr_decode_str(struct chr_cmd *cmd_set, u8 *data)
{
	...
	sprd_chr_get_cmdval(&cmd_set->evt_id, pos, "event_id=", 16);
 ...
 *   "wcn_chr_ind_event,module=WIFI,ref_count=%d,
 *   event_id=0x%x,version=0x%x,
 *   event_content_len=%d,
 *   char_info=*********"
*/
int sc2355_chr_sock_sendmsg(struct sprd_chr *chr, u8 *data)
{
	int ret;
...
void sprd_chr_rebuild_cmdlist(struct sprd_chr *chr, struct chr_cmd *cmd_set)
{
	u32 index;
...
static int sprd_chr_set_sockflag(struct sprd_chr *chr, u8 *data)
{
...
retry:
	...
	while (!kthread_should_stop() && chr->sock_flag != 2) {
		...
		if (sprd_hif_is_on(&priv->hif)) {
			...
			pr_info("%s, CHR: Drop set_chr_cmd in case of power off, save buf in chr_cmdlist",
				__func__);
		}
	}
...
int sc2355_init_chr(struct sprd_chr *chr)
{
...
void sc2355_deinit_chr(struct sprd_chr *chr)
{
...
struct sprd_chr_ops sc2355_chr_ops = {
	.init_chr = sc2355_init_chr,
	.deinit_chr = sc2355_deinit_chr,
	.chr_sock_sendmsg = sc2355_chr_sock_sendmsg,
	.chr_report_openerr = sc2355_report_chr_open_error,
};
#endif
