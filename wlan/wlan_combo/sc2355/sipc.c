...
*sipc_alloc_pcie_addr_buf(int tx_count)
{
	...
	mb();
	if (!addr_buffer) {
		pr_err("%s:%d alloc sipc addr buf fail\n", __func__, __LINE__);
		return NULL;
	}
...
static void sipc_tx_ba_mgmt(struct sprd_priv *priv, struct sprd_vif *vif,
			    void *data, int len, unsigned char cmd_id)
{
	...
	if (sprd_get_debug_level() >= L_INFO)
		sc2355_hex_dump("sipc_tx_ba_mgmt", data_ptr, len);
...
inline void sc2355_sipc_tx_addr_trans_free(struct sprd_hif *hif)
{
	...
	sc2355_sipc_tx_cmd_pop_list(hif->tx_data_port,
			       (struct mbuf_t *)rx_mgmt->addr_trans_head,
			       (struct mbuf_t *)rx_mgmt->addr_trans_tail,
			       rx_mgmt->addr_trans_num);
...
/*TODO*/
		...
		if (msg_pos->skb)
			sipc_skb_to_tx_buf(hif, msg_pos);
		pr_debug("debug sipc addr: 0x%lx\n", msg_pos->pcie_addr);
...
/*TODO*/
	...
	//dscr->tx_ctrl.sipc_mh_readcomp = 0;
			    ...
			    struct mbuf_t *tail, int num)
{
	...
	for (mbuf_pos = head; mbuf_pos != NULL;
		...
		if (mbuf_pos->buf != NULL) {
			...
			pr_info("%s error mbuf->pos is NULL!\n", __func__);
		if (--tmp_num == 0)
			break;
	}
					 ...
					 struct sprd_msg *msg_buf)
{
	...
	if (msg_buf->sipc_node)
		sipc_free_tx_buf(tx_msg->hif, (struct sipc_buf_node *)(msg_buf->sipc_node));
	if (msg_buf->skb)
...
int sc2355_tx_free_sipc_data(unsigned char *data)
{
	...
	for (i = 0; i < data_num; i++, pos++) {
		...
		pr_debug("%s: sipc_addr=0x%lx", __func__, sipc_addr);
...
{
	sipc_tx_ba_mgmt(vif->priv, vif, data, len, CMD_ADDBA_REQ);
}
...
void sc2355_sipc_tx_send_delba(struct sprd_vif *vif, void *data, int len)
{
	...
	sipc_tx_ba_mgmt(vif->priv, vif, delba,
