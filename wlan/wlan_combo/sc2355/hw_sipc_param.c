...
#define CF_TAB(NAME, MEM_OFFSET, TYPE) \
	{ NAME, (size_t)(&(((struct merl_wifi_conf_t *)(0))->MEM_OFFSET)), TYPE}

#define OFS_MARK_STRING \
	"#-----------------------------------------------------------------\r\n"

static struct merl_nvm_name_table g_config_table[] = {
	/* [Section 1: Version]
	 */
...
static int wifi_nvm_set_cmd(struct merl_nvm_name_table *pTable,
	struct merl_nvm_cali_cmd *cmd, void *p_data)
{
	int i;
	unsigned char *p;
...
static void get_cmd_par(char *str, struct merl_nvm_cali_cmd *cmd)
{
	...
	cType = 0;
	flag = 0;
	memset(cmd, 0, sizeof(struct merl_nvm_cali_cmd));
...
static struct merl_nvm_name_table *cf_table_match(struct merl_nvm_cali_cmd *cmd)
{
	int i;
	struct merl_nvm_name_table *pTable = NULL;
	int len = sizeof(g_config_table) / sizeof(struct merl_nvm_name_table);
...
{
	int i, p;
	struct merl_nvm_cali_cmd *cmd;
	struct merl_wifi_conf_t *conf;
	struct merl_nvm_name_table *pTable = NULL;

	if ((NULL == pBuf) || (0 == file_len))
		return -1;

	cmd = kzalloc(sizeof(struct merl_nvm_cali_cmd), GFP_KERNEL);
...
static int wifi_nvm_parse(struct sprd_priv *priv, const char *path, void *p_data)
{
