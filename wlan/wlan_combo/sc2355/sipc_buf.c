...
{
	int i;
	struct sprd_msg_list *list = &buf_mm->nlist;
...
void sipc_buf_mm_deinit(struct sprd_msg_list *list)
{
...
struct sipc_buf_node *sipc_alloc_node_buf(struct sprd_msg_list *list)
{
	struct sipc_buf_node *node = NULL;
...
void sipc_free_tx_buf(struct sprd_hif *hif,
						struct sipc_buf_node *node)
{
...
{
	struct sprd_msg_list *list = &mm->nlist;
...
static int sipc_init_tx_mm(struct sipc_txrx_mm *txrx_buf)
{
	...
	tx_buf->len =  ALIGN(SPRD_MAX_DATA_TXLEN + tx_buf->padding, ARCH_DMA_MINALIGN);
...
static int sipc_init_rx_mm(struct sipc_txrx_mm *txrx_buf)
{
	...
	rx_buf->len = ALIGN(SPRD_MAX_DATA_RXLEN + rx_buf->padding, ARCH_DMA_MINALIGN);
...
{
	struct sipc_buf_mm *buf_mm = hif->sipc_mm->rx_buf;
...
{
	struct sipc_txrx_mm *sipc_mm = hif->sipc_mm;
...
int sipc_skb_to_tx_buf(struct sprd_hif *hif,
				struct sprd_msg *msg_pos)
{
	...
	pad = (char *)node->buf + SPRD_MAX_DATA_TXLEN;
	memcpy_toio(pad, &node, sizeof(node));
	memcpy_toio(node->buf, skb->data, skb->len);
	phy_addr = ((unsigned long)node->buf - tx_buf->offset) | SPRD_MH_ADDRESS_BIT;
	phy_addr &= SPRD_MH_SIPC_ADDRESS_BIT;
				...
				struct sk_buff *skb)
{
	...
	rx_buf = hif->sipc_mm->rx_buf;
	memcpy_toio(&node, skb->data, sizeof(node));
	memcpy_fromio(skb->data, node->buf, SPRD_MAX_DATA_RXLEN);
...
{
	enum sprd_hif_type hw_type = hif->hw_type;

	if (SPRD_HW_SC2355_SIPC == hw_type)
