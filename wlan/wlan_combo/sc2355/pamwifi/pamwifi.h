...
int sprdwl_pamwifi_xmit_to_ipa(struct sk_buff *skb, 
	                     struct net_device *ndev);

void sprdwl_pamwifi_update_router_table(struct sprd_priv *priv, 
        void  *data,	u8 vif_mode, u32 index, int add);

void sprdwl_pamwifi_ul_resource_event(struct sprd_vif *vif, 
	 u8 *data, u16 len);

int  sprdwl_pamwifi_send_ul_res_cmd(struct sprd_priv *priv, 
	                     u8 vif_ctx_id,
				void *data, u16 len);
