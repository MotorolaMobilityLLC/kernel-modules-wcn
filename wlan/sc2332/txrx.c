		     ...
		     u8 type, u8 offset, bool flag)
{
	...
	if (type == SPRDWL_DATA_TYPE_WAPI) {
		if (sprdwl_rebuild_wapi_skb(vif, &skb)) {
			vif->priv->if_ops->free_msg_buf(vif->priv->hw_intf,
							msg);
			return -ENOMEM;
		}
	...
	buf = skb->data;
	skb_push(skb, sizeof(*hdr) + offset + vif->priv->hard_reserv_len);
	...
	ret = vif->priv->if_ops->tx(vif->priv->hw_intf, msg);
	if (ret) {
		if (type == SPRDWL_DATA_TYPE_WAPI)
			dev_kfree_skb(skb);
					...
					unsigned short len)
{
	...
	if (!skb) {
		vif->priv->if_ops->free_msg_buf(vif->priv->hw_intf, msg);
