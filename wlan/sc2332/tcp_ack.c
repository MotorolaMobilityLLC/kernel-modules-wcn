...
struct sprdwl_tcp_ack_info {
	int busy;
	int drop_cnt;
	/* psh_flag set to 1 when receive a ACK with
	 * TCP_FLAG_PUSH == 1
	 */
	int psh_flag;
	u32 psh_seq;
...
static void sprdwl_tcp_ack_timeout(unsigned long data)
{
	...
	if (msg) {
		...
		sprdwl_ack_manage.priv->if_ops->tx(sprdwl_ack_manage.priv->
						   hw_intf, msg);
		return;
	}
	spin_unlock_bh(&ack_info->lock);
 ...
 * type: 2 new_msgbuf must be send
 * 	 1 new_msgbuf is ACK and can be droped,
			  ...
			  int type)
{
	...
	if (type == 2) {
		if (SPRDWL_U32_BEFORE(ack->seq, ack_msg->seq)) {
			...
			ack_m->priv->if_ops->tcp_drop_msg(ack_info->msgbuf);
			ack_info->msgbuf = NULL;
			ack_info->drop_cnt = 0;
			spin_unlock_bh(&ack_info->lock);
