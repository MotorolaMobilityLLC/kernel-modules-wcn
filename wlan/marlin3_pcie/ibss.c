				     ...
				     struct cfg80211_ibss_params *params)
{
	...
	if (SPRDWL_MODE_IBSS != vif->mode) {
		netdev_err(ndev, "%s invalid mode: %d\n", __func__,
			   vif->mode);
		return -EINVAL;
	}
	...
	if (chan) {
		ret =  sprdwl_set_channel(vif->priv, vif->mode,
					  ...
					  chan->center_freq));
		if (ret < 0) {
			netdev_err(ndev, "%s set channel failed(%d)\n",
				   __func__, ret);
			return ret;
		}
	}
	...
	netdev_info(ndev, "%s params->ssid=%s\n", __func__, params->ssid);
	netdev_info(ndev, "%s params->ssid_len=%d\n",
	...
	netdev_info(ndev, "%s join_params.ssid=%s\n",
		    __func__, join_params.ssid);
	netdev_info(ndev, "%s join_params.ssid_len=%d\n",
	...
	if (ret) {
		netdev_err(ndev, "%s set coalesce failed (%d)\n",
			   __func__, ret);
		return ret;
	}
	...
	ret = sprdwl_set_ibss_attribute(vif->priv, vif->mode,
					...
					scan_suppress);
	if (ret) {
		netdev_err(ndev, "%s set scan_suppress failed (%d)\n",
			   __func__, ret);
		return ret;
	}
	...
	if (ret) {
		netdev_err(ndev, "%s set ATIM failed (%d)\n",
			   __func__, ret);
		return ret;
	}
	...
	if (ret) {
		netdev_err(ndev, "%s set wpa_version failed (%d)\n",
			   __func__, ret);
		return ret;
	}
#endif /* IBSS_RSN_SUPPORT */
	...
	if (ret) {
		netdev_err(ndev, "%s join failed (%d)\n", __func__, ret);
		return ret;
	}
				      ...
				      struct net_device *ndev)
{
	...
	if (ret < 0) {
		...
		netdev_err(ndev, "%s disconnect failed (%d)\n", __func__, ret);
		return ret;
	}
