...
static int mtty_sipc_rx_cb(int chn, struct mbuf_t *head, struct mbuf_t *tail, int num)
{
    ...
    struct rx_data *rx;
	len_send = head->len;
	dev_unisoc_bt_err(ttyBT_dev,"%s() bsp rxdata channel= %d ,length:%d\n", __func__,chn,len_send);
	for(i = 0;i < (head->len); i++) {
		dev_unisoc_bt_err(ttyBT_dev,"%s() bsp rxdata:%d\n", __func__,head->buf[i]);
	}
    ...
    // if(is_user_debug){
        // bt_host_data_save((unsigned char *)head->buf + BT_SIPC_HEAD_LEN, block_size, BT_DATA_IN);
    // }
...
static int mtty_sipc_tx_cb(int chn, struct mbuf_t *head, struct mbuf_t *tail, int num)
{
    ...
    {
		dev_unisoc_bt_err(ttyBT_dev,"%s sprdwcn_bus_list_free() success\n", __func__);
        up(&sem_id);
    }
    else
...
static int stty_open(struct tty_struct *tty, struct file *filp)
{
	...
	dev_unisoc_bt_err(ttyBT_dev,
							"debugw dst:%d, channel:%d\n",stty->pdata->dst,stty->pdata->channel);

    // for (i = 0; i < 100; i++ ){
		// ret = sbuf_status(stty->pdata->dst, stty->pdata->channel);
		// if (ret == 0){
		// dev_unisoc_bt_err(ttyBT_dev,
	...
	dev_unisoc_bt_err(ttyBT_dev,"debugw sleep finish\n");

	dev_unisoc_bt_info(ttyBT_dev,"stty_open device success1!\n");
	stty->tty = tty;
	tty->driver_data = (void *)stty;

	mtty = tty;
	dev_unisoc_bt_info(ttyBT_dev,
						"stty_open device success2!\n");
	//stty->stty_state = STTY_STATE_OPEN;
	...
	dev_unisoc_bt_info(ttyBT_dev,
						"stty_open device success3!\n");
	sitm_ini();
#if 0
	stty_address_init();
#endif

	//if (wcn_hw_type == HW_TYPE_SIPC) {
...
static void stty_close(struct tty_struct *tty, struct file *filp)
{
	...
	//stty->stty_state = STTY_STATE_CLOSE;

	dev_unisoc_bt_info(ttyBT_dev,
						"stty_close device success !\n");

	//if (wcn_hw_type == HW_TYPE_SIPC) {
		//mtty_dma_buf_free(BT_SIPC_RX_MAX_NUM);
            ...
            const unsigned char *buf, int count)
{
	...
	dev_unisoc_bt_err(ttyBT_dev,"debugw:%s down start\n",__func__);

    down(&sem_id);

	dev_unisoc_bt_err(ttyBT_dev,"debugw:%s down successful\n",__func__);
	...
	dev_unisoc_bt_err(ttyBT_dev,"debugw:%s sprdwcn_bus_list_alloc ended\n",__func__);

	for(i = 0;i < count;i++){
		dev_unisoc_bt_err(ttyBT_dev,"debugw:%s() tx_head->buf: %d\n", __func__, tx_head->buf[i]);
	}

	dev_unisoc_bt_err(ttyBT_dev,"debugw:%s() tx_head: %p, tx_tail: %p, num: %d\n", __func__, tx_head, tx_tail, num);
	...
	dev_unisoc_bt_err(ttyBT_dev,
						"debugw:%s sprdwcn_bus_push_list ended\n",
						__func__);
...
static int  stty_probe(struct platform_device *pdev)
{
	...
	dev_unisoc_bt_info(ttyBT_dev,"debugw:mtty_probe successful!\n");
