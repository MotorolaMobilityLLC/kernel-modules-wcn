...
{
	cpu_latency_qos_update_request(pm_qos, CPU_DMA_FORBID_POWER_DOWN_LATENCY);
}

static void wcn_pm_qos_cpu_pd_allow(struct pm_qos_request *pm_qos)
{
	cpu_latency_qos_update_request(pm_qos, PM_QOS_CPU_LATENCY_DEFAULT_VALUE);
}
...
static int wcn_freq_pm_qos_init(void)
{
	...
	for (i = CPU_CORE_NUM_OFFSET; i < ARRAY_SIZE(pqos->wcn_freq_qos_req); i++, j++) {
		...
		WCN_INFO("%s: CPU%d->%d min_freq=%d,max_freq=%d,requset freq=%d\n", __func__,
...
int wcn_pm_qos_init(void)
{
	...
	cpu_latency_qos_update_request(&pqos->pm_qos_req, PM_QOS_CPU_LATENCY_DEFAULT_VALUE);
...
failed:
	cpu_latency_qos_remove_request(&pqos->pm_qos_req);
	wpq = NULL;
	kfree(pqos);
	return ret;
}
