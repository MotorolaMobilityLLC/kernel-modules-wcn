...
static int fm_sipc_send_cmd(unsigned char subcmd, void *payload, int payload_len) {
    ...
    dev_unisoc_fm_err(fm_miscdev,"%s() debugw: Enter fm_sipc_send_cmd :%d\n", __func__);
    ...
    for(i = 0;i < (cmd_hdr->len); i++) {
        dev_unisoc_fm_err(fm_miscdev,"%s() fm_sipc_send_cmd txdata:%d\n", __func__,cmd_hdr[i]);
    }
...
static void receive_tasklet(unsigned long arg)
{
    ...
    while (!list_empty(&fmdev->rx_head)) {
        ...
        parse_sipc_header(head, tail, num, sipc_hdr);
		//if (wcn_hw_type == HW_TYPE_SDIO) {
			if (sipc_hdr->subtype != FM_SIPC_RX_CHANNEL - 12) {
				dev_unisoc_fm_info(fm_miscdev,"%s sub type is wrong [%d]\n", __func__, sipc_hdr->subtype);
				kfree(sipc_hdr);
				sipc_hdr = NULL;
				return;
			}
...
static int fm_sipc_rx_cback(int chn, struct mbuf_t *head,struct mbuf_t *tail, int num)
{
	...
	for(i = 0;i < (head->len); i++) {
		dev_unisoc_fm_err(fm_miscdev,"%s() fm_sipc_rx_cback rxdata:%d\n", __func__,head->buf[i]);
	}
#ifdef FM_DUMP_DATA
